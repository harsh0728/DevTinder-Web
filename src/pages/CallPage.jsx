...
const CallPage = () => {
  ...
  // ✅ ICE server config with TURN fallback for production NAT traversal.
  // STUN alone fails behind most corporate/mobile NATs in production.
  // TURN credentials should come from your env / a credential API in production.
  // Free TURN options: Metered.ca, Xirsys, Twilio, or self-host coturn.
  // ✅ Only include TURN entries when credentials are actually present.
  // An empty string username/credential causes InvalidAccessError crash in the browser.
  // To enable TURN: add VITE_TURN_USERNAME + VITE_TURN_CREDENTIAL to your .env
  // Free TURN: https://www.metered.ca/tools/openrelay | Twilio | self-host coturn
  const _turnUser = import.meta.env.VITE_TURN_USERNAME;
  const _turnPass = import.meta.env.VITE_TURN_CREDENTIAL;
  const _hasTurn = Boolean(_turnUser && _turnPass);
  ...
  const iceServers = {
    iceServers: [
      ...
      ...(_hasTurn
        ? [
            { urls: "turn:a.relay.metered.ca:80",               username: _turnUser, credential: _turnPass },
            { urls: "turn:a.relay.metered.ca:443",              username: _turnUser, credential: _turnPass },
            { urls: "turn:a.relay.metered.ca:443?transport=tcp", username: _turnUser, credential: _turnPass },
          ]
        : []),
    ],
  };

  // ✅ Must match getCallRoomId on the server — "call:" prefix prevents collision with chat rooms
  const getSecretRoomId = (userId1, userId2) => {
    const ids = [userId1, userId2].sort().join("_");
    return "call:" + crypto.SHA256(ids).toString(crypto.enc.Hex);
  };
  ...
  const initializeLocalStream = async (isVideoCall = true) => {
    // ✅ Don't re-initialize if already have a stream
    if (localStreamRef.current) {
      // ✅ But still (re-)attach to ref in case the video element mounted AFTER
      // the stream was initialized (common in production with slower renders)
  ...
  useEffect(() => {
    ...
    // ─── Auto-initiate ONLY if this client is explicitly the caller ─────────
    //
    // ❌ Old approach: both users open CallPage and we use a 500ms grace timer
    //    to detect who the receiver is. This races and causes double-initiation.
    //
    // ✅ New approach: the chat page appends ?role=caller when navigating to CallPage
    //    for the person who clicked "Call". The receiver navigates WITHOUT that param.
    //    No timing race — role is determined at navigation time.
    //
    //    In your ChatPage / wherever you start a call:
    //      navigate(`/call/${myUserId}/${theirUserId}?role=caller`);
    //
    ...
    if (isCaller) {
      ...
      const initiateCall = async () => {
        try {
          // ✅ Read call type from ?type=video|audio set by the chat page
          const callTypeParam = new URLSearchParams(window.location.search).get("type");
          const isVideoCall = callTypeParam !== "audio"; // default to video
          setCallType(isVideoCall ? "video" : "audio");
    ...
    // Receivers do nothing here — they wait for the "incoming-call" socket event
